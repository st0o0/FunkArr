...
public sealed class ScoringHistoryWorkerTests() : TestKit(_persistenceConfig)
{
    ...
    private static IOptionsMonitor<ScoringHistoryOptions> HistoryOpts(int maxSnapshots = 100, int maxAgeDays = 30, int snapshotInterval = 20) =>
        new TestOptionsMonitor<ScoringHistoryOptions>(new ScoringHistoryOptions
        {
    ...
    private IActorRef CreateWorker(string ruleSetId, int maxSnapshots = 100, int maxAgeDays = 30, int snapshotInterval = 20) =>
        Sys.ActorOf(Props.Create(() => new ScoringHistoryWorker(HistoryOpts(maxSnapshots, maxAgeDays, snapshotInterval), ruleSetId)));

    private static RecordScoring CreateRecord(string ruleSetId, Guid? requestId = null, DateTimeOffset? timestamp = null) =>
        new(
    ...
    public void Query_detail_returns_not_found_for_unknown_request()
    {
        ...
        var result = ExpectMsg<ScoringDetailFailed>();
        Assert.IsType<KeyNotFoundException>(result.Cause);
    }
