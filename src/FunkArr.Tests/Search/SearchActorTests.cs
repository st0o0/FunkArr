...
public sealed class SearchActorTests : Akka.Hosting.TestKit.TestKit
{
    ...
    protected override void ConfigureAkka(AkkaConfigurationBuilder builder, IServiceProvider provider)
    {
        ...
        builder.WithActors((system, registry, _) =>
        {
            var ruleSetStub = system.ActorOf(Props.Create(() =>
                new StubRuleSetRegistry(request => _rulesProvider!(request.TvdbId))));
            registry.Register<RuleSetRegistryActor>(ruleSetStub);

            var matchLedgerStub = system.ActorOf(Props.Create(() =>
                new StubMatchLedger(record => _recordedMatches!.Enqueue(record))));
            registry.Register<MatchLedgerActor>(matchLedgerStub);
        });
    }
    ...
    private static TvdbClient CreateTvdbClient() =>
        new(new HttpClient(new FakeHttpMessageHandler(
            _ => FakeHttpMessageHandler.JsonResponse("{}"))) { BaseAddress = new Uri("https://api.thetvdb.com/") });
    ...
    private IActorRef CreateActor(MediathekClient mediathekClient, TvdbClient? tvdbClient = null)
    {
        ...
        return Sys.ActorOf(Props.Create(() => new SearchActor(mediathekClient, tvdb, qualityProbeService, options)));
    }
    ...
    public async Task TvSearch_CacheMissRoutesToChildAndCachesReply()
    {
        ...
        var request = new SearchActor.TvSearchRequest(1, "Tatort", null, null, "Tatort");

        var first = await actor.Ask<SearchActor.SearchResponse>(request, TimeSpan.FromSeconds(3));
        Assert.Single(first.Results);
        Assert.Equal(1, callCount);

        await AwaitAssertAsync(() => Assert.Single(_recordedMatches!));

        var second = await actor.Ask<SearchActor.SearchResponse>(request, TimeSpan.FromSeconds(3));
    ...
    public async Task TvSearch_MatchRecordForwardedToMatchLedger()
    {
        ...
        var request = new SearchActor.TvSearchRequest(1, "Tatort", null, null, "Tatort");
        await actor.Ask<SearchActor.SearchResponse>(request, TimeSpan.FromSeconds(3));
    ...
    public async Task Requests_AreEventuallyServedAfterDependencyResolution()
    {
        ...
        var request = new SearchActor.TextSearchRequest("Beliebiger Titel");
        var response = await actor.Ask<SearchActor.SearchResponse>(request, TimeSpan.FromSeconds(3));
    ...
    {
        public StubRuleSetRegistry(Func<RuleSetRegistryActor.GetRulesForTopic, IReadOnlyList<Rule>> rulesProvider)
        {
            Receive<RuleSetRegistryActor.GetRulesForTopic>(msg =>
                Sender.Tell(new RuleSetRegistryActor.RulesResponse(rulesProvider(msg))));
        }
    }

    private sealed class StubMatchLedger : ReceiveActor
    {
        public StubMatchLedger(Action<MatchRecord> onRecord)
        {
            Receive<MatchLedgerActor.RecordMatchResult>(msg => onRecord(msg.Record));
        }
    }
