...
public sealed class MatchQualityActor : ReceivePersistentActor, IWithTimers
{
    ...
    {
        Recover<MatchRecordedJournal>(dto => Apply(dto.ToDomain()));
        Recover<MatchesExpiredJournal>(dto => Apply(dto.ToDomain()));
        ...
        {
            _log.Info("MatchQualityActor recovery completed. {Count} records", _records.Count);
            Become(Ready);
    ...
    private void RunEviction()
    {
        ...
        if (_records.Count == 0 || _records.Last!.Value.Timestamp >= threshold)
        {
            return;
        }
