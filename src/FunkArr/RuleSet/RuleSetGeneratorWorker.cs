...
public sealed partial class RuleSetGeneratorWorker : ReceiveActor
{
    ...
    public RuleSetGeneratorWorker(MediathekClient mediathekClient)
    {
    ...
    private async Task HandleGenerateAsync(GenerateRuleSet request)
    {
        try
        {
            ...
            if (results.Length == 0)
            {
                ...
                Context.Parent.Tell(new RuleSetCoordinator.GenerationFailed(request.TvdbId));
                Context.Stop(Self);
                return;
            }
            ...
            Context.Parent.Tell(new RuleSetCoordinator.GenerationComplete(ruleSet));
        }
        catch (Exception ex)
        {
