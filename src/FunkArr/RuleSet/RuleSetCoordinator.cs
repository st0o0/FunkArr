...
public sealed class RuleSetCoordinator : ReceiveActor, IWithTimers
{
    ...
    private IActorRef _refreshWorker = ActorRefs.Nobody;
    private IActorRef _matchQualityWorker = ActorRefs.Nobody;

    public RuleSetCoordinator(
        ...
        IOptions<RuleSetOptions> options)
    {
        ...
        _refreshWorker = Context.ActorOf(Props.Create(() =>
            new RefreshWorker(gitHubReleaseClient)), "refresh");
        _matchQualityWorker = Context.ActorOf(Props.Create(() =>
            new MatchQualityWorker()), "match-quality");
        ...
        Receive<RefreshCommunity>(_ =>
            _refreshWorker.Tell(new RefreshWorker.DoRefresh(_communityPath)));
        Receive<RefreshWorker.RefreshComplete>(HandleRefreshComplete);
        ...
        Receive<MatchQualityWorker.RecordMatchResult>(msg =>
            _matchQualityWorker.Forward(msg));
        Receive<MatchQualityWorker.GetRecentMatches>(msg =>
            _matchQualityWorker.Forward(msg));
        Receive<MatchQualityWorker.GetTopicStats>(msg =>
            _matchQualityWorker.Forward(msg));
        Receive<MatchQualityWorker.GetAllTopicStats>(msg =>
            _matchQualityWorker.Forward(msg));
        Receive<MatchQualityWorker.GetUnmatchedItems>(msg =>
            _matchQualityWorker.Forward(msg));
    }

    private void HandleRefreshComplete(RefreshWorker.RefreshComplete msg)
    {
        if (msg.Updated)
        {
            LoadAllFromDisk();
    ...
    private void HandleGetRulesForTopic(GetRulesForTopic request)
    {
        ...
        else if (request.TvdbId is { } id && _generationInProgress.Add(id))
        {
            _log.Info(
                ...
                request.Topic,
                id);
            var generator = Context.ResolveChildActor<RuleSetGeneratorWorker>(
                $"generator-{id}");
            generator.Tell(new RuleSetGeneratorWorker.GenerateRuleSet(id, request.Topic));
        }
