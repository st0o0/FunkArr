...
public static partial class MatchingPipeline
{
    ...
    private static IEnumerable<SearchResult> ExpandQualities(MediathekResultItem item)
    {
        ...
        {
            yield return CreateResult(item, item.UrlVideoHd, QualityTier.HD1080, timestamp);
        }

        if (!string.IsNullOrEmpty(item.UrlVideo))
        {
            yield return CreateResult(item, item.UrlVideo, QualityTier.HD720, timestamp);
        }

        if (!string.IsNullOrEmpty(item.UrlVideoLow))
        {
            yield return CreateResult(item, item.UrlVideoLow, QualityTier.SD, timestamp);
        }
    }
    ...
    private static void AddQualityVariants(List<SearchResult> results, MediathekResultItem item)
    {
        ...
        if (!string.IsNullOrEmpty(item.UrlVideoHd))
        {
            results.Add(CreateResult(item, item.UrlVideoHd, QualityTier.HD1080, timestamp));
        }

        if (!string.IsNullOrEmpty(item.UrlVideo))
        {
            results.Add(CreateResult(item, item.UrlVideo, QualityTier.HD720, timestamp));
        }

        if (!string.IsNullOrEmpty(item.UrlVideoLow))
        {
            results.Add(CreateResult(item, item.UrlVideoLow, QualityTier.SD, timestamp));
        }
    }
