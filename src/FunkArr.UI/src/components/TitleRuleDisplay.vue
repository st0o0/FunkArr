  ...
  <div class="space-y-1 text-xs">
    <div v-for="(rule, idx) in titleRules" :key="idx" class="flex items-start gap-2 py-0.5">
      <span class="text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0"
        :class="rule.type === TitlePartType.Static ? 'bg-surface-elevated text-text-secondary' : 'bg-accent/10 text-accent'"
      >{{ titlePartLabel(rule.type, t) }}</span>
      <template v-if="rule.type === TitlePartType.Static">
...
import { TitlePartType } from '../api/enumMaps'
