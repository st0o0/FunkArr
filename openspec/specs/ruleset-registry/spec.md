...
The `RuleSetActor` SHALL handle a `GetAllRulesets` message and respond with metadata for every registered topic: topic name, source, rule count, media reference, and aliases.
...
The `RuleSetActor` SHALL handle a `GetRuleSet` message and respond with the full RuleSetFile for the requested topic.
...
The `RuleSetActor` SHALL handle a `SaveLocalRuleSet` message that writes a RuleSetFile to `data/rulesets/local/` and reloads the in-memory index.
...
The `RuleSetActor` SHALL handle a `DeleteLocalRuleSet` message that removes a local override file and reloads.
...
The `RuleSetActor` SHALL handle a `TestRules` message by searching the Mediathek for the topic, optionally fetching TVDB episodes, running `RuleSetMatchingEngine.EvaluateRulesWithTraces`, and returning the trace results.
