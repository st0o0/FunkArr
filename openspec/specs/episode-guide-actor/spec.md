...
EnrichmentManager singleton actor handling episode and movie resolution requests with provider-specific data caching.

## Requirements

### Requirement: EnrichmentManager is a Cluster Singleton
The EnrichmentManager SHALL be registered as a Cluster Singleton actor. It SHALL be resolvable via `context.GetActor<IEnrichmentManager>()`. The project namespace SHALL be `FunkArr.Enrichment`.

#### Scenario: Singleton registration
- **WHEN** the actor system starts
- **THEN** the EnrichmentManager SHALL be registered as a Cluster Singleton with the key `IEnrichmentManager`

#### Scenario: Actor resolution
- **WHEN** another actor calls `Context.GetActor<IEnrichmentManager>()`
- **THEN** it SHALL receive a reference to the EnrichmentManager singleton

### Requirement: EnrichmentManager uses Router Pool pattern
The EnrichmentManager SHALL create provider-specific worker pools using `SmallestMailboxPool` routers (same pattern as ScoringManager). A TVDB pool (`TvdbEnrichmentActor`, pool size 2) SHALL handle episode resolution API calls. A TMDB pool (`TmdbEnrichmentActor`, pool size 2) SHALL handle movie resolution API calls. This allows TVDB and TMDB calls to execute in parallel without blocking each other.
...
- **THEN** it SHALL send a CacheUpdate message to the parent EnrichmentManager actor to store the fetched data
...
- **THEN** the EnrichmentManager SHALL respond with `EpisodeResolutionFailed("TVDB API key not configured")` directly (no pool involvement)

#### Scenario: Resolution strategy is "none"
- **WHEN** `ResolveEpisodes` is received with ResolutionConfig.Strategy="none"
- **THEN** the EnrichmentManager SHALL respond with an empty `EpisodesResolved` without fetching TVDB data

### Requirement: EnrichmentManager handles ResolveMovie
The EnrichmentManager SHALL handle `ResolveMovie` messages by fetching TMDB movie data (from cache or API), applying movie resolution strategies, and responding with `MoviesResolved` or `MovieResolutionFailed`.
...
### Requirement: EnrichmentManager passes through regex-extracted episodes
When candidates already have ExistingSeason and ExistingEpisode set (from regex extraction in the scoring engine), the EnrichmentManager SHALL include them in the response as-is with Strategy="RegexExtracted" and Confidence=1.0, without querying TVDB for those items.
...
### Requirement: EnrichmentManager manages metadata cache
The EnrichmentManager SHALL maintain a unified in-memory cache for both TVDB and TMDB data, keyed by (provider, id). Cache TTLs SHALL be content-aware: active TV shows 2 days, inactive TV shows 7 days, movies 30 days.
...
### Requirement: EnrichmentManager handles timeouts gracefully
The EnrichmentManager SHALL apply a timeout to TVDB and TMDB API calls. If the timeout is exceeded, the resolver SHALL respond with the appropriate failure message.

#### Scenario: TVDB API timeout
- **WHEN** the TVDB API does not respond within the configured timeout (default 10 seconds)
- **THEN** the EnrichmentManager SHALL respond with `EpisodeResolutionFailed("TVDB API timeout")`
