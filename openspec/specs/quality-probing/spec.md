...
QualityProbeService provides multi-phase quality detection for Mediathek video URLs: Phase 1 (URL pattern analysis via `UrlPatternAnalyzer`), Phase 2 (lightweight HTTP probing via `Mp4AtomParser` or `HlsManifestParser`). Results are cached in-memory with configurable TTL.
